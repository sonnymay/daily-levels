...
- **Crash-resilient earned progress** — completed focus checkpoints enter a tiny on-device recovery journal before SwiftData saves them. Entries decode independently, timestamps repair damaged duration fields, stable record IDs make replay idempotent, and the journal clears only after a successful save. Unusable or out-of-range entries are ignored, while partially damaged journals keep every recoverable record ([`FocusJournal.swift`](DailyLevels/FocusJournal.swift), [`FocusJournalTests.swift`](DailyLevelsTests/FocusJournalTests.swift)).
- **Atomic locked-focus recovery** — the active checkpoint and lock confirmation are encoded as one local marker, so interrupted writes cannot pair a stale timestamp with a new lock state. Existing two-key markers migrate automatically. Recovery requires finite timestamps and at least one whole second after the daily cap is applied; malformed, unlocked, or unsafe markers are discarded without awarding time ([`ActiveFocusMarker.swift`](DailyLevels/ActiveFocusMarker.swift), [`TrustAuditTests.swift`](DailyLevelsTests/TrustAuditTests.swift)).
- **Interval-backed history** — daily totals, personal bests, and lifetime progress are derived from each persisted session's start/end timestamps. Duplicate and overlapping intervals are collapsed into one timeline, redundant duration values cannot inflate focus, and non-finite or integer-out-of-range intervals are ignored without hiding valid neighboring sessions.
- **Deterministic focus accounting** — `FocusEngine` reads one injectable wall-clock source for Start, Pause, Resume, timer ticks, and lifecycle callbacks. Production uses the system clock; tests pin exact instants to prove paused gaps never count. Non-finite or unrepresentable live clock samples preserve the last trustworthy boundary, and focus totals saturate instead of overflowing ([`FocusEngineTransitionTests.swift`](DailyLevelsTests/FocusEngineTransitionTests.swift)).
...
├── ActiveFocusMarker.swift    # atomic locked-focus recovery marker      ← unit-tested
...
DailyLevelsTests/              # unit tests for math, midnight, i18n, and trust boundaries
