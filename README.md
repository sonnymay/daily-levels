...
- **Deterministic focus accounting** — `FocusEngine` reads one injectable wall-clock source for Start, Pause, Resume, timer ticks, and lifecycle callbacks. Production uses the system clock; tests pin exact instants to prove paused gaps never count ([`FocusEngineTransitionTests.swift`](DailyLevelsTests/FocusEngineTransitionTests.swift)).
