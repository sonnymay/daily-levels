...
- **Crash-resilient earned progress** — completed focus checkpoints enter a tiny on-device recovery journal before SwiftData saves them. Entries decode independently, stable record IDs make replay idempotent, and the journal clears only after a successful save, so one damaged entry or a storage interruption cannot silently erase healthy earned focus ([`FocusJournal.swift`](DailyLevels/FocusJournal.swift), [`FocusJournalTests.swift`](DailyLevelsTests/FocusJournalTests.swift)).
