...
device via SwiftData; there is no account, no backend, no network calls, no analytics, no ads,
no third-party SDKs.
...
- **None required.** The app uses no camera, microphone, location, contacts, photos,
  notifications, or HealthKit. No `Info.plist` usage-description keys are needed.
